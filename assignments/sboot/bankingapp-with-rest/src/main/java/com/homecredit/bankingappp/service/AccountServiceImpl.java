...
public class AccountServiceImpl implements AccountService {
    ...
    public boolean create(Account account)  throws ApplicationException {
        return accountDao.create(account);
    }

    public Account get(int id)  throws ApplicationException {
    ...
    public long getNoOFAccountBalanceMoreThan1Lac() throws ApplicationException  {
        long count = accountDao.getAll().stream().filter(acc -> acc.getBalance() > 100000).count();
        return count;
    }

    public Map<String , Long> getNoOfAccountByAccountType() throws ApplicationException{
        return accountDao.getAll().stream().map(Account::getType).collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    }

    public Map<String , Long> getNoOfAccountByAccountTypeWithSorting() throws ApplicationException {
        return accountDao.getAll()
                .stream()
    ...
    public Map<String , Double> getAvgBalanceByAccountType() throws ApplicationException {
        return accountDao.getAll()
                .stream()
    ...
    public List<Integer> getListOfAccountIdsWithGivenName(String name) throws ApplicationException{
	...
	public synchronized void bulkImport()  throws ApplicationException {
		int counter = 0;
...
//				account.setAccountId(Integer.parseInt(tokenizer.nextToken()));
	...
	public void bulkExport() throws ApplicationException {
